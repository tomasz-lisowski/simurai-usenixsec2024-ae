...
tmux split-window -h;
tmux send-keys 'cd /opt/setup/firmwire && python3 -u ./firmwire.py /firmwire/modem_files/XT1970-3_KANE_RETEU_11_RSA31.Q1-48-36-23.bin 2>&1 | tee /opt/setup/log/firmwire.log' 'C-m';
tmux -2 attach-session -t main;

printf "\\n\\n4. Checking FirmWire log for SIM communication.\\n";
read -p "Press enter to run step...";

N_RAPDU=`grep -a "PCSC RAPDU" /opt/setup/log/firmwire.log  | wc -l`
N_CAPDU=`grep -a "PCSC CAPDU" /opt/setup/log/firmwire.log  | wc -l`

printf "\n\nFound %d C-APDUs and %d R-APDUs!\n" $N_CAPDU $N_RAPDU
printf "Checking for SIMurais ICCID...\n"

grep -a "PCSC RAPDU" /opt/setup/log/firmwire.log  | grep $SIMURAI_ICCID > /dev/null
